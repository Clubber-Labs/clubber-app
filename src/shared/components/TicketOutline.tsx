...
import { NOTCH_RADIUS } from './TicketPerforation'
...
type Props = {
  ...
  notchY: number
  // O mesmo raio passado ao TicketPerforation, senão furo e tracejado divergem.
  notchRadius?: number
...
/**
 * A aresta do card-ingresso. Existe porque `border` do RN não serve aqui: filho
 * de View com borda é inset pela largura dela, então o furo nascia 1px pra
 * dentro e a linha passava reta por fora, sem acompanhar o recorte — e cobrir a
 * borda pelo filho diverge entre iOS (CALayer desenha por cima) e Android (por
 * baixo).
 *
...
export function TicketOutline({
  ...
  return (
    ...
    >
      ...
      <Circle cx={0} cy={notchY} r={notchRadius} fill={colors.background} />
      <Circle cx={width} cy={notchY} r={notchRadius} fill={colors.background} />
      <Path
        d={ticketOutlinePath({
          width,
          height,
          ...
          radius: radius - STROKE_INSET,
          inset: STROKE_INSET,
          notch: { y: notchY, radius: notchRadius },
        })}
        fill="none"
        stroke={colors.line}
        strokeWidth={1}
